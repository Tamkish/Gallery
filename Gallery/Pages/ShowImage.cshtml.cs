...
namespace Gallery.Pages
{
    public class ShowImageModel : PageModel
    {
        ...
        public ShowImageModel(ILogger<ShowImageModel> logger, IWebHostEnvironment environment,
        ...
        public string LoadComments(Comment cc)
        {
            ...
            if (cc.HasComments)
            {
                ...
                output += " <ul>";
                foreach (var ccc in cc.Children) output += LoadComments(ccc);
                output += " </ul>";
            }
        ...
        {
            var photoGuid = new Guid(PhotoId);
            Error = null;
            ...
            var album = _context.Albums
                .Where(a => (a.OwnerId == userId || a.Public) && a.Files.Any(a => a.Id == photoGuid)).FirstOrDefault();
            if (album == null)
            {
                Error = "no album";
                return Page();
            ...
            photosrc = Photo.Id + ".";
            var extention = Photo.ContentType.Split("/").Last();
            ...
            HtmlComments = "<ul>";
            foreach (var c in comments) HtmlComments += LoadComments(c);
            HtmlComments += "</ul>";
